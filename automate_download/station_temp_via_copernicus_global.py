...
with open("stationClimateERA5_global_{0}_{1}.csv".format(datetime.now().year, datetime.now().strftime('%m')), 'w', encoding="UTF8") as csvFile:
...
#			'year': list(range(1950,2024)),
		...
		for i in range(0, len(precip)):
			...
			csvWriter.writerow([station["Id"], station["Name"], station["Latitude"], station["Longitude"], converteddates[i].year, converteddates[i].month, temp2m[i][0][0][0]-273.15, precip[i][0][0][0]])
